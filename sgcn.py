...
# If you are using google colab please uncomment these lins
"""from google.colab import drive
drive.mount('/content/drive')


# %cd '/content/drive/My Drive/SGCN/keras-gcn/'
...
data_path = '/content/drive/My Drive/SGCN/'

if flag_dataset==0:
  davis_data=io.loadmat(data_path + 'davis.mat')
  ...
  kiba_data=io.loadmat(data_path + 'KIBA.mat')
  ...
  bindingdb_data=io.loadmat(data_path + 'KIBA.mat')
...
def generate_data(batch_size,flag):
  ...
  while True:
        ...
        model_Semi.fit([np.array(input1),np.array(input2), Adj],np.array(output1),sample_weight=np.concatenate((np.zeros((batch_size,)),np.ones((10*batch_size,)))),
                 batch_size=10*batch_size+batch_size, epochs=10, shuffle=False, verbose = False)
...
for epoch in range(0, NB_EPOCH+1):
    ...
    history = model_Semi.fit_generator(generator=generate_data(batch_size,1),
              steps_per_epoch = num_train_samples/batch_size, epochs=10, shuffle=False, verbose=1)#,validation_data=generate_data_val(batch_size1), validation_steps=50)
