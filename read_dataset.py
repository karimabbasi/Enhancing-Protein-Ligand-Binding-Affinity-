...
####################################################################################
##   many part of this code is taken from DeepDTA repository (datahelper.py file)  #
####################################################################################
...
class DataSet(object):
  ...
  def parse_data(self, FLAGS,  with_label=True): 
    ...
    drug_drug = np.loadtxt(fpath + "kiba_drug_sim"+".txt")#davis "drug-drug_similarities_2D"+".txt")
    protein_protein = np.loadtxt(fpath + "kiba_target_sim"+".txt")
    import csv
    with open('/content/drive/My Drive/Project/Project/DeepDTI/MultiDTI/kiba/drug_drug.csv', 'w', encoding='UTF8', newline='') as f:
        writer = csv.writer(f)

        # write multiple rows
        writer.writerows(drug_drug)
    with open('/content/drive/My Drive/Project/Project/DeepDTI/MultiDTI/kiba/protein_protein.csv', 'w', encoding='UTF8', newline='') as f:
        writer = csv.writer(f)

        # write multiple rows
        writer.writerows(protein_protein)
...
FLAGS = argparser()
FLAGS.amino_acids_counts = 25
FLAGS.masks_counts = 15
FLAGS.max_seq_len = 1000
FLAGS.max_smi_len = 100
FLAGS.problem_type = 1
FLAGS.dataset_name = 'davis'    # set it to 'davis' or 'kiba'
FLAGS.dataset_path = '/content/drive/My Drive/'  # set it to the path of the dataset
